...
lettuce = Ingredient("lettuce", 1 / 6, "head")
...
nutrients = [
    ...
    IngredientGrp(broccoli, brussel_sprouts, artichoke_hearts, pref=3),
    IngredientGrp(green_beans),
    IngredientGrp(peas, pref=5 if month() in winter | spring else 3),
    IngredientGrp(chard, pref=4 if month() in summer else 1),
...
carbs = [
    ...
    IngredientGrp(pasta, pref=6),
    IngredientGrp(quinoa),
    IngredientGrp(couscous, pref=3),
...
def get_weighted_selection(options: List[IngredientGrp], n=1):
    ...
    return [
        Selection(result, servings=NUM_MEALS * NUM_PEOPLE / n) for result in results
    ]
...
def make_meal_selections(num_meals):
    ...
    grocery_list_str = "\n".join(
        f"{item.with_serving(item.serving*cnt)}"
        + (f"(used in {cnt} meals)" if cnt > 1 else "")
